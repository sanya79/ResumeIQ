...
import { matchPipelineSteps, type MatchPipelineStepId, type MatchStepStatus } from "../useMatchPipeline";

interface ProcessingTimelineProps {
  statuses: Record<MatchPipelineStepId, MatchStepStatus>;
}

export function ProcessingTimeline({ statuses }: ProcessingTimelineProps) {
  const statusMap: Record<MatchStepStatus, TimelineStep["status"]> = {
    pending: "upcoming",
    active: "current",
    complete: "complete",
  };

  const steps: TimelineStep[] = matchPipelineSteps.map((step) => ({
    title: step.label,
  ...
  return (
    <FadeIn>
      <GlassCard glow className="mx-auto flex max-w-lg flex-col items-center gap-6 p-10 text-center">
        ...
        <div>
          <h2 className="text-fluid-lg font-semibold">AI is matching your resume</h2>
