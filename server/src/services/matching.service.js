...
export class MatchingService {
  async analyzeJobMatch(userId, resumeId, jobDescription, jobTitle = "Software Engineer", company = "Target Company") {
    ...
    // 1. Scan job description for core tech keywords
    const jdKeywords = this._extractKeywordsFromJd(jobDescription);
    ...
    const matchScore = totalJdWeight > 0 ? Math.round((matchCount / totalJdWeight) * 100) : 75; // fallback to 75 if no keywords detected
